...
function startEducatorStream(loggedInId) {
  if (loggedInId) {
    currentEducId = loggedInId;
  }
  ...
  var absoluteStreamUrl =
    "https://sse-stat.amrit-corp.com/" + encodeURIComponent(currentEducId);
...
function saveAndNotify(senderName, messageText, action) {
  ...
  if (action === "classroom_comment") {
    rfshcmnt();
  }
  if (action === "student_request") {
    allstudwait();
  }
}
...
function createNotificationUI() {
  ...
  document.getElementById("educlassroom").appendChild(badgeBtn);
  ...
  modal.innerHTML = `
    ...
    <div class="modal-controls">
      <button id="stream-toggle-btn" onclick="startEducatorStream()">Start Stream</button>
  ...
  document.getElementById("educlassroom").appendChild(modal);
  updateStreamButtonUI();
}
