...
function deltopictod(label) {
  ...
  var ur2 =
    "AKfycbygqnfRauECR-vPi39Nsy7ymyUYSwfsrbFBSNxjtV1W43ZE4bzX0glwsy7eeiIwU8Z-DA";
  var urlrmv =
    ur1 +
    ur2 +
    "/exec" +
...
strtodedu.addEventListener("submit", (event) => {
  ...
  if (flag === 0) {
    var ur1 = "https://script.google.com/macros/s/";
    var ur3 =
      "AKfycbxg7rXgSOFW9q0OCYH3wqviTxNMAf1KPOsDyDkdC-URdZ2myYoUx18s6okzyVu2LCmu";
    var url =
      ur1 +
      ur3 +
      "/exec" +
  ...
  if (res != "ID not found!") {
    if (res[0].AllTOD != 0) {
      ...
      document.getElementById(
        "prevsttod"
      ).innerHTML = `<div style="padding:40px;"> <svg xmlns="http://www.w3.org/2000/svg" style="color:#8a8a8b;" width="60" height="60" fill="currentColor" class="bi bi-info-circle" viewBox="0 0 16 16">
...
srcandsvex.addEventListener("submit", (event) => {
  ...
  if (nsvexm === 1) {
    ...
    document.getElementById("svexminfo").disabled = false;
  }
  $.getJSON(
      ...
      for (var i = 0; i < json.records.length - 1; i++) {
        if (
          ...
          document.getElementById("examdescpin").innerHTML =
            "<div><p style='text-align:left;'><i>Educator:</i> <b>" +
            json.records[i].EducatorName +
            "</b><br><i>Exam Title:</i> <b>" +
            json.records[i].ExamTitle +
            "</b><br><i>Description:</i> <b>" +
...
function shoeprevexresult(label) {
  ...
  $.getJSON(
    ...
    function (json) {
      for (var i = 0; i < json.records.length - 1; i++) {
        if (
          examid === json.records[i].ExamID &&
          enpass === json.records[i].ExamPass
        ) {
          var restren = JSON.parse(
            JSON.stringify(json.records[i].EnrolledStuFinal)
          );
